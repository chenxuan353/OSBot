...
from ...os_bot_base.util import strip_control_characters
...
def deal_trans_text(text):
    ...
    text = strip_control_characters(text)  # 移除控制字符
    # text = EmojiFilter.filter(text)  # 移除emoji
    text = emoji_match.sub('', text)
    return text
