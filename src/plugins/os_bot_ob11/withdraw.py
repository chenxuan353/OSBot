"""
    ...
    撤回机能修改自[nonebot-plugin-withdraw](https://github.com/noneplugin/nonebot-plugin-withdraw)
...
PermManage.register("撤回", "允许群成员使用撤回功能", auth=False, only_super_oprate=True)
...
async def save_msg_id_v11(bot: BaseBot, e: Optional[Exception], api: str,
...
async def save_msg_id_v12(bot: BaseBot, e: Optional[Exception], api: str,
                          data: Dict[str, Any], result: Any):
    try:
...
withdraw = on_command("withdraw",
                      ...
                      block=True,
                      rule=smart_to_me,
                      permission=perm_check_permission("撤回") | SUPERUSER
                      | GROUP_OWNER | GROUP_ADMIN | PRIVATE_FRIEND)
...
async def _(
    ...
    if isinstance(event, V11MEvent):
        ...
        id = str(
            event.group_id if isinstance(event, V11GMEvent) else event.user_id)
    else:
...
@withdraw_notice.handle()
def _(bot: V12Bot, event: Union[GroupMessageDeleteEvent,
                                ChannelMessageDeleteEvent]):
