...
from ..os_bot_base.depends import Adapter, AdapterDepend, BotRecord, OBCacheBotDepend, ArgMatchDepend, OBCacheOBGroupDepend
...
set_group_card = on_command("设置群名片", permission=SUPERUSER)
...
async def _(matcher: Matcher,
            ...
            group: GroupRecord = OBCacheOBGroupDepend(),
    ...
    await matcher.finish(
        f"已将{group.get_nick()}({group.id})内咱的名片设置为{card or '与昵称一致'}")
...
async def _(matcher: Matcher,
            ...
            adapter: Adapter = AdapterDepend()):
    group: Optional[GroupRecord] = cache.get_group_record(arg.group_id)
    if not group:
