...
// 外部参数
var GLOBAL_TOOL = typeof playwright_config != "undefined" && playwright_config || {
...
(function () {
    ...
    const CSSAnchor = {
        ...
        // 报错检查
        twitterError() {
            let dom = document.querySelector("div[data-testid=error-detail] DIV>span");
            if(!dom){
    ...
    (function () {
            ...
            */
            ...
            try {
                ...
                let insertTextData = function (
                    ...
                    if (cover_flag) {
                        sourcedom.hidden = true;
                    } else {
                        sourcedom.hidden = false;
                    }
        ...
        TweetHtml.removeAllInsert = function () {
            ...
            $("." + MARK_HIDE_CLASS.replace(/\s+/g, ".")).attr("hidden", false);
        };
        // 文本方式解析
                    ...
                    if (markitem.mark == "level") {
                        ...
                        if(markitem.default()==="template"){
                            ...
                            if(!trans[markitem.result]){
                                // 默认模版不覆盖已有值
                                trans[markitem.result] = markitem.content.trim();
                            }
                        }else{
    ...
    function tweetCoreInit() {
        ...
        let consoleTest = function (text, template = "<p>翻译自日语</p>") {
             ...
             */
            transSwitch(true);
            return TweetHtml.insertTrans(null, TweetHtml.parsingArgStr(text, template));
        };
...
if (GLOBAL_TOOL.ENABLE_PLAYWRIGHT) {
    function twitterWaitLoad(timeout) {
        return new Promise((resolve) => {
            return GLOBAL_TOOL.TweetHtml.waitLoad((isOK, reason) => resolve([isOK, reason]), timeout);
        });
    }
